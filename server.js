...
app.post('/api/onboarding', async (req, res) => {
  ...
  const attachments = docs
    .map(filename => {
      ...
      return { filename, path: filepath, content: fs.readFileSync(filepath).toString('base64'), type: 'application/pdf', disposition: 'attachment' };
    })
    .filter(Boolean);
...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:32px 0;"><tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:white;border-radius:10px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08);">
      ...
      <tr><td style="padding:36px 40px;">
        <p style="color:#333;font-size:15px;line-height:1.7;margin:0 0 20px">Nous sommes ravis de t'accueillir au sein de l'équipe du <strong>${magasinLabel}</strong>, au poste de <strong>${departement.toLowerCase()}</strong>.</p>
        <p style="color:#333;font-size:15px;line-height:1.7;margin:0 0 28px">Tu trouveras en pièces jointes à ce courriel tous les documents importants pour ton intégration :</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#fff5f5;border-radius:8px;padding:4px 0;margin-bottom:28px;">
          ${attachments.map(a => `<tr><td style="padding:10px 20px;border-bottom:1px solid #ffe0e0;"><span style="color:#e31837;font-size:13px;">&#128196; ${a.filename}</span></td></tr>`).join('')}
        </table>
        <p style="color:#333;font-size:15px;line-height:1.7;margin:0 0 12px"><strong>Important :</strong> Lors de ton premier jour, n'oublie pas d'apporter un spécimen de chèque avec les documents complétés que tu nous remettras.</p>
        <p style="color:#333;font-size:15px;line-height:1.7;margin:0 0 12px">Si tu as des questions avant ton premier jour, n'hésite pas à communiquer avec ton gestionnaire.</p>
        <p style="color:#333;font-size:15px;line-height:1.7;margin:0">On a hâte de te voir !</p>
      </td></tr>
      <tr><td style="padding:0 40px 36px;"><div style="border-top:1px solid #eee;padding-top:24px;"><p style="margin:0;color:#e31837;font-weight:700;font-size:14px">L'équipe Marchés Riendeau</p><p style="margin:4px 0 0;color:#888;font-size:12px">${magasinLabel}</p></div></td></tr>
      <tr><td style="background:#f0f0f0;padding:16px 40px;text-align:center;"><p style="margin:0;color:#aaa;font-size:11px">Ce courriel vous a été envoyé car vous avez consenti à recevoir des communications de Marchés Riendeau.</p></td></tr>
    </table>
  </td></tr></table>
</body></html>`;
  ...
  try {
    await sgMail.send({ to: courriel, from: process.env.FROM_EMAIL, subject: `Bienvenue chez ${magasinLabel}, ${prenom}!`, html: htmlBody, attachments });
    let employeeId = null;
    const existing = findEmployeeByEmail(courriel, magasin);
    if (existing) { employeeId = existing.id; }
    else { const result = stmts.insertEmployee(prenom, nom, courriel, magasin, departement, new Date().toISOString().slice(0, 10), 'probation'); employeeId = result.lastInsertRowid; }
    ...
    try { const { stmts: s } = require('./database'); s.insertLog(null, prenom, nom, courriel, magasin, departement, JSON.stringify(docs), 'erreur', null); } catch (e) {}
